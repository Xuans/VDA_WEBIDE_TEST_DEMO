...
const getAllRoutes=(data)=>{
    let seed=[{parent:'/',data:data}],i=-1,item,ret=[];

    while(item=seed[++i]){
        
      for(let _i=-1,_item;_item=item.data[++_i];){
        let parent=`${item.parent}/${_item.path}`;
        ret.push(parent.replace(/(\/)+/g,'/'))
        if(_item.children){
            seed.push({parent:parent,data:_item.children})
        }
      }
...
export default {
    ...
    methods: {
        ...
        async open(option) {
            ...
            let allRoutes=getAllRoutes(routerTemp[0].children);
            
            let hasIn = allRoutes.some(item=>item===path); //判断是否存在路由表里
